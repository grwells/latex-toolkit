...
project_path = nil
flag_silent = nil -- set to enable debug statements
...
-- ]]
local parser = argparse():name("ltk"):add_complete():description([[
██╗      █████╗ ████████╗███████╗██╗  ██╗              ████████╗██╗  ██╗
██║     ██╔══██╗╚══██╔══╝██╔════╝╚██╗██╔╝              ╚══██╔══╝██║ ██╔╝
██║     ███████║   ██║   █████╗   ╚███╔╝     █████╗       ██║   █████╔╝ 
██║     ██╔══██║   ██║   ██╔══╝   ██╔██╗     ╚════╝       ██║   ██╔═██╗ 
...
parser
	...
	:action(function(args)
		...
		local project_name = infer_project_name({ silent = true })

		logger:info(string.format("output parts: \n\t`%s` \n\t`%s` \n\t`%s`", project_name, ver, date_str))

		-- write document to output file in project root
		local fn_str = string.format("%s_%s_%s.pdf", project_name, ver, date_str)
		logger:info("exporting file -> ./" .. fn_str)
...
parser
	...
	:action(function(args, _, fn)
		...
		if ret ~= 0 then
			...
			logger:error("nothing to commit aborting tag creation")
		else
			-- get the latest tag
			-- most recent tag `git describe --tags --abbrev=0 # 0.1.0-dev
			-- most recent annotated tag `git describe --abbrev=0`
